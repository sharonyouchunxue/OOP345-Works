...
namespace sdds {
   ...
   ConfirmOrder& ConfirmOrder::operator+=(const Toy& toy) {
      ...
      //resize the array to make room for toy if it is not found
      if (!found) {
         ...
         delete[] m_toy;
         m_toy = temp;
         //stores the address of toy in the array
         temp[m_noOfRecs] = &toy;
         ++m_noOfRecs;
      }
      return *this;
   ...
   //searches the array for the address of toy, sets the pointer 
   //in the array to nullptr if toy is found.
   ConfirmOrder& ConfirmOrder::operator-=(const Toy& toy) { 
      size_t count = 0;
      for (size_t i = 0; i < m_noOfRecs; ++i) {
         if (m_toy[i] == &toy) {
   ...
   //Friend Helpers
   //overload the insertion operator to insert the content of a ConfirmOrder object into 
   //an ostream object: 
